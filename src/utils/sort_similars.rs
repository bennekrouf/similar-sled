use crate::models::similar_output::SimilarOutput;
...
pub fn sort_similars(similars: &mut [SimilarOutput]) {
    similars.sort_by(|a, b| {
        ...
        if verse_len_cmp == std::cmp::Ordering::Equal {
            let a_chapter_sum: u32 = a.verses.iter().map(|verse| verse.chapter_no).sum();
            let b_chapter_sum: u32 = b.verses.iter().map(|verse| verse.chapter_no).sum();
