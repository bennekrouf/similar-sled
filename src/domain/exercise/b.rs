...
pub fn generate(dbs: &Database, kalima: String) -> Option<Exercise> {
    ...
    let valid_sourate = exercise.verses[valid_verse_index].verse.sourate.clone();
    let valid_ayah = exercise.verses[valid_verse_index].verse.clone();
    ...
    let alternatives = extract_and_shuffle_options(&mut exercise.verses, 
        ...
        &Some(Alternative {
            // content: valid_sourate.as_ref().unwrap_or(&"".to_string()).clone(),
            verse: Some(valid_ayah.clone())
        }));

    let mut distinct_alternatives = deduplicate_by_field(alternatives, |alt| Some(alt.verse.as_ref().unwrap().sourate.clone()));
    ...
    let valid_statement = &mut exercise.verses[valid_verse_index];
    if let Some(sourate) = &mut valid_statement.verse.sourate {
        sourate.clear();
    }
    valid_statement.verse.chapter_no = 0; // You can set this to any default value
    //valid_statement.chapter_no = 0; // Set it to a default value

    // Step 5: Add the selected chapter data back to the list
    distinct_alternatives.push(Alternative {
        // content: valid_sourate.unwrap_or_default(),
        verse: Some(valid_ayah),
    });
     Some(Exercise {
        statement: valid_statement.clone(),
